...
| **P4a — Scoped invitations + bindings API** — ✅ **SHIPPED v0.5.38** (2026-05-15) | v0.5.38 | `invitations.scope_payload` JSONB column (via `_PENDING_COLUMNS`); `mint_invitation()` accepts and validates scope_payload; `redeem_invitation()` creates role_bindings from payload; API endpoints: `POST /invitations` with scope_payload, `GET/POST/DELETE /users/<id>/bindings` for binding management. All super_admin gated. Audit events for grants/revokes. | A5 (backend) + A6 (API) | Yes — schema additive, API endpoints use existing role decorators. |
| **P4b — Invite form + bindings UI** | v0.5.39 | Invite form gains site-multi-select + group/device pickers (HTML); per-user binding editor at `/app/users/<id>` showing current bindings + add/remove UI; per-site members tab on `/app/sites/<id>`. Builds on P4a API. | A5 (UI) + A6 (UI) + A7 | Yes — UI-only, no schema changes. |
