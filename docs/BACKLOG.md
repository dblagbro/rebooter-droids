...
| v0.5.101 | **Remediation Group A** — three fixes from today's risk-prioritised remediation plan: (a) **form-input hardening** — closed the latent uncaught-`ValueError` (since v0.5.67) at all 7 sites in `rules.py` + `schedules.py` via a new `_int_field` helper in `_common.py`; non-numeric input now flashes + redirects (form) or returns `validation_failed` (JSON) instead of 500; (b) **probe-card backfill** on `/app/rules/<id>/edit` from 9 of 26 `KNOWN_PROBE_KINDS` to all 26 (added the 6 network kinds + 10 integration kinds); (c) **archived `docs/qa-notes.md`** (~105 K-token soak diary) to `docs/sessions/qa-notes-archived-20260519.md`. 6 new live regression tests pin every fixed site. Gate ~856. |
| v0.5.102 | **Remediation Group B — B11 sync.** Big de-risking finding mid-flight: the "B11 applier is a stub" framing in `design.md §6` was 30+ versions stale — applier shipped fully v0.5.70-.72 with 18 gated tests. Group B reframed to operator tooling, not implementation: (a) `design.md §6 + §7` + `BACKLOG.md` historical B11 entry refreshed to reflect actual state; (b) new `docs/runbooks/sync-enable.md` operator playbook (3-phase preflight + 24-h soak + kill switch + rollback); (c) new `GET /api/v1/admin/sync/status` admin-Bearer wrapper around the HMAC-only wire endpoint — caught + fixed a pre-commit field-name bug (production-truth peer field is `url`, not `base_url`); (d) new `scripts/sync-dual-hub-preflight.sh` operator harness; (e) 6 new gated tests. **Post-deploy verification surfaced**: `sync.enabled=true` already on BOTH hubs since 2026-05-16, fully converged in both directions (max_seq=14↔cursor=14 + max_seq=5↔cursor=5, last_error=null). The runbook applies as "verify-what's-running" rather than first-time-flip. Gate ~862. |
