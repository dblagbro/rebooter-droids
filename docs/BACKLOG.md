...
| **B11 bootstrap-seam** — independently-bootstrapped entities don't converge | small | **open, not a blocker.** Each hub bootstraps its own admin user (same email) and "Default" site (same name) with *different* ids. On sync these collide on the `users.email` / `sites.name` unique constraints — the event is logged + skipped (sync does not wedge), but those specific rows never converge. Fix: reconcile by natural key, or don't double-bootstrap. Surfaces on the sync cursor's `last_error`. |
