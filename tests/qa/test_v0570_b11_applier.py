...
from app.models import Base, Site
...
def test_lww_stale_write_is_skipped(session):
    newer = T0 + timedelta(minutes=5)
    session.add(Site(id="site_c", name="Current", created_at=T0, updated_at=newer))
    session.flush()
...
def test_delete_event_removes_row_and_writes_tombstone(session):
    session.add(Site(id="site_e", name="Doomed", created_at=T0, updated_at=T0))
    session.flush()
