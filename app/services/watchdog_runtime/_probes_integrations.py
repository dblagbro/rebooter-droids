"""Integration / sensor watchdog probes — v0.5.64 (split from `_probes.py`).

The sensor-backed probe implementations: Roku, Home Assistant, weather,
iCal, power (B16), solar + SNMP (P2), media/webhook (B17 Ship 2), MQTT
(Ship 3), and EPG (Layer 2). Split out of `_probes.py` so the core
network probes (internet / ping / tcp / http / dns) and the `run_probe`
dispatcher stay small and navigable.

`run_probe` in `_probes.py` dispatches a probe kind to the function
here. These probes read samples via deferred service imports
(`external_sensors` / `device_power` / `epg`) — no DB writes, no
module-level service imports, so this module stays cycle-free.
"""
